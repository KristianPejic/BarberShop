<x-app-layout>
    ...
    <div class="container mx-auto bg-white p-5 m-10">
        <h1 class="text-2xl font-bold mb-4">Vaše rezervacije</h1>

        @if ($reservations->isEmpty())
            <p>Trenutno nemate nikakvih rezervacija!</p>
        @else
            <div id="reservation-list" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                @foreach ($reservations as $reservation)
                    <div id="reservation-{{ $reservation->id }}" class="reservation-card border p-4 rounded-lg shadow">
                        <h2 class="font-semibold text-lg mb-2">Usluga: {{ $reservation->service }}</h2>
                        <p class="text-black-700">Ime: {{$reservation->user->name}}</p>
                        <p class="text-gray-700">Frizer: {{ $reservation->barber }}</p>
                        <p class="text-gray-700">Datum: {{ $reservation->date }}</p>
                        <p class="text-gray-700">Vrijeme: {{ $reservation->time }}</p>
